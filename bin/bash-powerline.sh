...
__powerline() {
    ...
    then
        ...
        readonly BG_BASE1="\[$(tput setab 16)\]"
    ...
    __git_info() {
        ...
        # branch is modified?
        # TODO if modified: make branch font RED: else GREEN
    ...
    ps1() {
        ...
        GIT_INFO=$(__git_info)
        if [[ "$?" == 0 ]]; then
            PS1_YES_GIT="$BG_BLUE$FG_BLACK$PS_SYMBOL$RESET"
            PS1+="$PS1_YES_GIT"
            PS1+="$BG_BLUE$FG_BASE3$GIT_INFO$RESET"
            PS1_END_GIT="$BG_BASE03$FG_BLUE$PS_SYMBOL$RESET"
            PS1+="$PS1_END_GIT"
        else
            PS1_NO_GIT="$BG_BASE03$FG_BLACK$PS_SYMBOL$RESET"
            PS1+="$PS1_NO_GIT"
        fi
